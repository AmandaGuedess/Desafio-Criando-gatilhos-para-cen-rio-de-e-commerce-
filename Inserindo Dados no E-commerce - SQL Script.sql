...
CREATE TABLE IF NOT EXISTS HistoricoCliente (
    ...
    idTipoCliente INT,
    DataDeExclusao TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);

-- 2. Criar o Trigger `before delete`
DELIMITER $$
...
FOR EACH ROW
BEGIN
    INSERT INTO HistoricoCliente (idCliente, Pnome, NomeDoMeio, Sobrenome, CPF, Endereco, DataDeNascimento, idTipoCliente)
    VALUES (OLD.idCliente, OLD.Pnome, OLD.NomeDoMeio, OLD.Sobrenome, OLD.CPF, OLD.Endereco, OLD.DataDeNascimento, OLD.idTipoCliente);
END$$
...
-- 1. Criar a Tabela de Histórico de Salários
CREATE TABLE IF NOT EXISTS HistoricoSalario (
    idHistoricoSalario INT AUTO_INCREMENT PRIMARY KEY,
    idColaborador INT,
    SalarioAntigo DECIMAL(10,2),
    DataDeAlteracao TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);

-- 2. Criar o Trigger `before update`
DELIMITER $$
...
FOR EACH ROW
BEGIN
    -- Inserir o salário antigo no histórico antes de atualizar o novo salário
    IF OLD.Salario <> NEW.Salario THEN
        INSERT INTO HistoricoSalario (idColaborador, SalarioAntigo)
        VALUES (OLD.idColaborador, OLD.Salario);
    END IF;
END$$
